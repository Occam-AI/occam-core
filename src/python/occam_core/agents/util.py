...
class OccamLLMMessage(OccamDataType):
    ...
    requires_source_attachment: Optional[bool] = None
    """
    This flag indicates whether the message requires a source attachment.
    Only valid for messages that are of attachment type. This is set to
    True only for messages of type ATTACHMENT, that are read from an
    already saved attachment.
    - It's None for all message types other than ATTACHMENT.
    - It's False for all messages of type ATTACHMENT that are not read from an already saved attachment
    (e.g. generated by an agent)
    - It's True for messages of type ATTACHMENT that are read from an already saved attachment.
    """
    ...
    def validate_messages(self):
        # If requires_source_attachment is not set, it defaults to True for attachment messages
        if self.requires_source_attachment is None and self.type == MessageType.ATTACHMENT.value:
            self.requires_source_attachment = True
        if self.type != MessageType.ATTACHMENT.value and self.requires_source_attachment:
            raise ValueError("Messages that require a source attachment must be of type ATTACHMENT.")
        if self.type == MessageType.ATTACHMENT.value:
            ...
            if self.requires_source_attachment and not self.source_attachment:
                raise ValueError("Attachment messages that require a source attachment must have a source attachment.")
